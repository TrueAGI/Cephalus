...
from cephalus.kernel import StateKernel, StateKernelFrame, StatePredictionProvider
...
class StandardStatePredictionProvider(StatePredictionProvider):
    ...
    losses provided by the environment and other kernel modules, with no intrinsic loss. The state
    model is cloned directly from the configuration's model template without modification."""
    ...
    def predict_state(self, decision_frame: StateKernelFrame) -> Optional[tf.Tensor]:
        return self._state_model([decision_frame.previous_state, decision_frame.input_tensor])
